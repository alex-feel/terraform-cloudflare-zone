...
| <a name="input_early_hints"></a> [early\_hints](#input\_early\_hints) | When enabled, Cloudflare will attempt to speed up overall page loads by serving 103 responses with Link headers from the final response (https://developers.cloudflare.com/cache/about/early-hints). | `string` | `"off"` | no |
...
| <a name="input_orange_to_orange"></a> [orange\_to\_orange](#input\_orange\_to\_orange) | Orange to Orange (O2O) allows zones on Cloudflare to CNAME to other zones also on Cloudflare. | `string` | `"off"` | no |
...
| <a name="input_proxy_read_timeout"></a> [proxy\_read\_timeout](#input\_proxy\_read\_timeout) | Maximum time between two read operations from origin. | `number` | `100` | no |
